# -*- coding: utf-8 -*-
"""
Created on Mon Dec 31 10:27:42 2018
周末是股票的休息日，所以最小单位是5天

我们关注具有一定时间间隔的联动，比如A股票上涨或下跌了，在时间间隔1天或2天后，
B股票与其的联动关系。这样所获得的规则才有助于实际的操作。我们简称这种有时间间
隔的联动为延时联动，而相应的涨跌联动规则称为股票延时联动规则。

A股票大涨/大跌->在间隔时间后，B股票大张/大跌的概率。

定义三种时间间隔：1日间隔、2日间隔和3日间隔

为简化计算规模，规则里定义大涨是指涨幅超过2%，大跌是指跌幅超过-2%

联动关系的应用最好再结合全景数据的分析，先大盘再板块后个股

参考  https://stocktobe.com/book/paper.aspx?id=5&token=
...
def rules2file(m, n, core, data, flag=1):
    ...
    for i in list(np.arange(1, len(data)+1)):
        ...
        values.append(value)
    if flag == 1:
        file = '../result/2.1/大涨/' + str(m) + '_' + str(n) + '_' + str(core) + '.txt'
        ...
        file = '../result/2.1/大跌/' + str(m) + '_' + str(n) + '_' + str(core) + '.txt'
        with open(file, 'wt') as f:
            for i in range(len(data)):
                for j in range(len(data)):
                    if i == j:
