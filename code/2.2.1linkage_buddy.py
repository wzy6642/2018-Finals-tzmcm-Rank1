...
def show_inc_dec(file, m):
    ...
    for i in range(len(file)):
        ...
        for j in range(len(y)):
            if y[j] == 1:
                plt.bar(x[j], y[j], fc='r', alpha=0.5)
            elif y[j] == 0:
                plt.bar(x[j], y[j], fc='c', alpha=0.5)
            else:
                plt.bar(x[j], y[j], fc='g', alpha=0.5)
        if m == 0:
            title = '大板块' + str(i+1) + '的股票大涨大跌情况的可视化'
        else:
            title = '大板块' + str(m) + '中的小板块' + str(i+1) + '的股票大涨大跌情况的可视化'
        plt.title(title)
        plt.xlabel('时间')
        plt.ylabel('涨跌情况')
        plt.grid(ls='--')
...
def rules2file(m, n, core, zuhe, data, flag=1):
    values = []
    for i in list(np.arange(1, len(data)+1)):
        ...
        values.append(value)
    if flag == 1:
        file = '../result/2.2/大涨/' + str(m) + '_' + str(n) + '_' + str(core) + '_' + str(zuhe[0]) + str(zuhe[1]) + str(zuhe[2]) + '.txt'
        with open(file, 'wt') as f:
            print('组合板块为：%d, %d, %d' % (zuhe[0], zuhe[1], zuhe[2]), file=f)
            for i in range(len(data)):
                for j in range(len(data)):
                    if i == j:
                        pass
        ...
        file = '../result/2.2/大跌/' + str(m) + '_' + str(n) + '_' + str(core) + '_' + str(zuhe[0]) + str(zuhe[1]) + str(zuhe[2]) + '.txt'
        with open(file, 'wt') as f:
            print('组合板块为：%d, %d, %d' % (zuhe[0], zuhe[1], zuhe[2]), file=f)
            for i in range(len(data)):
                for j in range(len(data)):
...
if __name__ == '__main__':
    ...
    core = 4
    rules2file(num1, num2, core, i, linkage_dec, flag=0)
###############################################################
## 这是训练集上的程序
## 程序备注：
## num1： 可选值有1  5  22  分别代表计算涨幅/跌幅的时间跨度为日  周  月
## num2： 可选值有1         计算轮动规律的间隔日期
## 499行中 sim_samp1可以替换为sim_samp2 sim_samp3 sim_samp4 分别代表对4大板块内部的小板块进行分析
## 499行中 sim_samp1的索引值可以填入0~9的数字，分别代表对哪一个联动板块分析
## 506行中 core可以取值为1~4 1：第一个小板块  2：第二个小板块  3：第三个小板块  4：第四个小板块
## 507行中 flag和linkage_dec一起改变
