# -*- coding: utf-8 -*-
"""
Created on Mon Dec 31 21:15:48 2018
...
def rules2file(m, n, core, zuhe, data, flag=1):
    ...
    for i in list(np.arange(1, len(data)+1)):
        ...
        values.append(value)
    if flag == 1:
        file = '../result/2.2/大涨/验证' + str(m) + '_' + str(n) + '_' + str(core) + '_' + str(zuhe[0]) + str(zuhe[1]) + str(zuhe[2]) + '.txt'
        ...
        file = '../result/2.2/大跌/验证' + str(m) + '_' + str(n) + '_' + str(core) + '_' + str(zuhe[0]) + str(zuhe[1]) + str(zuhe[2]) + '.txt'
        with open(file, 'wt') as f:
            print('组合板块为：%d, %d, %d' % (zuhe[0], zuhe[1], zuhe[2]), file=f)
            for i in range(len(data)):
                for j in range(len(data)):
...
## 490~493行中的3代表KNN的K为3，可以做适当的修改，亦可以做后处理
