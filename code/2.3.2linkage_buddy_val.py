...
def rules2file(N, m, n, core, zuhe, data, flag=1):
    ...
    for i in list(np.arange(1, len(data)+1)):
        ...
        values.append(value)
    if flag == 1:
        file = '../result/2.3/大涨/验证' + str(m) + '_' + str(n) + '_' + str(core) + '_' + str(zuhe[0]) + str(zuhe[1]) + str(zuhe[2]) + '_' + str(N) + '.txt'
        ...
        file = '../result/2.3/大跌/验证' + str(m) + '_' + str(n) + '_' + str(core) + '_' + str(zuhe[0]) + str(zuhe[1]) + str(zuhe[2]) + '_' + str(N) + '.txt'
        with open(file, 'wt') as f:
            print('时间跨%d组，组合板块为：%d, %d, %d' % (N, zuhe[0], zuhe[1], zuhe[2]), file=f)
            for i in range(len(data)):
                for j in range(len(data)):
...
if __name__ == '__main__':
    ...
    core = 1
    rules2file(N, num1, num2, core, i, linkage_inc, flag=1)
###############################################################
## 程序备注：
## N：时间跨度（组）
